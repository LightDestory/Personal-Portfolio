        ...
        if (timelineContainer && collection.timeline) {
            ...
            (collection.timeline as TimelineCategory[]).forEach(cat => {
                html += `<div class="timeline-category">
                            <h3 class="category-title section-title">${cat.category}</h3>
                ...
                cat.items.forEach(item => {
                    ...
                    if (isPublication) {
                        if (item.authors) {
                            html += `<div class="timeline-authors"><strong>Authors:</strong> ${item.authors}</div>`;
                        }
                        if (item.link && item.link !== "#") {
                            html += `<div class="timeline-link"><a href="${item.link}" target="_blank">View Publication <i class="fas fa-external-link-alt"></i></a></div>`;
                        }
                        if (item.abstract) {
